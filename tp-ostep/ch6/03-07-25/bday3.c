...
int main(){
    ...
    increment_age3(pYour_age); // [[ BOTH OF THESE WORK ]]
    increment_age3(&your_age); // [[ BOTH OF THESE WORK ]]
...
void increment_age3(int* pYour_age){ 
    // BIG NOTE: a function accepting pointer ie int*
    // you can simply pass in an address, not a pointer?
    printf("[increment_age3()] ptr_addy pre-inc %d\n", (*pYour_age));
    // pYour_age++; // only increments memory addy
    (*pYour_age)++; // increments value at memory addy
    printf("[increment_age3()] ptr_addy post-inc %d\n", (*pYour_age));
